...
parser.add_argument('--fair_scaling_batchsize', default=64, type=int)
...
def train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, dataset_info=None, args=None, attr_index=0, loss_scaler=None, group_dataloaders=[]):
    global device
...
if __name__ == '__main__':
    ...
    train_havo_dataset = FairVision_Glaucoma(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type, needBalance=args.need_balance, dataset_proportion=args.dataset_proportion)
    val_havo_dataset = FairVision_Glaucoma(os.path.join(args.data_dir, 'val'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
    test_havo_dataset = FairVision_Glaucoma(os.path.join(args.data_dir, 'test'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type)
    ...
    for i in range(groups_in_attrs[attr_to_idx[args.attribute_type]]):
        ...
        group_havo_dataset = FairVision_Glaucoma(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type, select_group=i)
    ...
    if args.model_type == 'ViT-B':
        ...
        # loss_scaler = Fair_Loss_Scaler_(fair_scaling_coef=args.fair_scaling_coef, sinkhorn_blur=args.fair_scaling_sinkhorn_blur,
        #                                 fair_scaling_temperature=args.fair_scaling_temperature)
    ...
    for epoch in range(start_epoch, args.epochs):
        train_loss, train_acc, train_auc, train_preds, train_gts, train_attrs = train(model, criterion, optimizer, scaler, train_dataset_loader, epoch, total_iteration, dataset_info=ds_info, args=args, attr_index=attribute_index, loss_scaler=loss_scaler, group_dataloaders=group_dataloaders)
        ...
        val_es_acc, val_es_auc, val_aucs_by_attrs, val_dpds, val_eods, val_between_group_disparity = evalute_comprehensive_perf(val_preds, val_gts, val_attrs)
        ...
        # test_acc_std, test_es_acc_std, test_auc_std, test_es_auc_std, test_aucs_by_attrs_std, test_dpds_std, test_eods_std, test_between_group_disparity_std = bootstrap_performance(test_preds, test_gts, test_attrs, num_classes=2, bootstrap_repeat_times=args.bootstrap_repeat_times)
        ...
        # logger.log(test_acc, test_es_acc, test_auc, test_es_auc, test_aucs_by_attrs, test_dpds, test_eods, test_between_group_disparity)
        ...
        # logger.log(test_acc_std, test_es_acc_std, test_auc_std, test_es_auc_std, test_aucs_by_attrs_std, test_dpds_std, test_eods_std, test_between_group_disparity_std)
        # sys.exit()
    ...
    best_acc_std, best_es_acc_std, best_auc_std, best_es_auc_std, best_auc_groups_std, best_dpd_groups_std, best_eod_groups_std, best_between_group_disparity_std = bootstrap_performance(best_test_preds, best_test_gts, best_test_attrs, num_classes=2, bootstrap_repeat_times=args.bootstrap_repeat_times)
    logger.log(f'(mean) best_acc, best_es_acc, best_auc, best_es_auc, best_auc_groups, best_dpd_groups, best_eod_groups, best_between_group_disparity')
